    ...
    for i, s in enumerate(items):
        ...
        if idx == sel:
            ...
            try:
                ...
                win.addstr(y, 29, f'{size_str:<7}', curses.color_pair(C_DIM))
                win.addstr(y, 37, f'{img_total_str:<10}', curses.color_pair(C_CYAN if img_total > 0 else C_DIM))
                win.addstr(y, 48, f'{mem_str:<9}', curses.color_pair(C_YELLOW if mem_str else C_DIM))
                win.addstr(y, 58, f'{status}', curses.color_pair(color))
            except: pass
