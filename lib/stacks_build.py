#!/usr/bin/env python3
"""
stacks_build.py — Interactive service scaffolder for BellzServer
...
CONF_DIR   = "/home/loveiznothin/.config/stacks"
...
def setup_db(db_type, svc_name):
    ...
    if choice.startswith("USE"):
        ...
        db_pass = ask("DB password",       "bellzpass")
...
def build_svc(name, image, ip, port, cfg, svc_num, db=None, redis=None):
    ...
    env_b = ("    environment:\n" + "\n".join(env) + "\n") if env else ""
    vols  = [f'      - "/home/loveiznothin/docker/{name}:/data"']
...
{caps}    image: {image}
    container_name: {name}
    hostname: {name}
    domainname: {name}.loveiznothin.com
    mac_address: "{mac}"
...
def inject(stack, block, network, volume=None):
    ...
    if volume and volume not in content:
        content = re.sub(r"^(volumes:\n)",
                        ...
                        content,count=1,flags=re.MULTILINE)
    if "##BELLZART_START_FOOTER" in content:
        content = content.replace("##BELLZART_START_FOOTER",
...
def main():
    ...
    # Write log
    lpath = f"/home/bellzserver/MyDocker/stacks_build_{svc_name}.log"
