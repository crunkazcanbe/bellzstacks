...
dyn_dir  = sys.argv[3] if len(sys.argv) > 3 else "/home/bellzserver/MyDocker/Configs/Dynamics"
conf_path = "/home/loveiznothin/.config/stacks/bellzart.conf"

# Load art from bellzart.conf
header_art = ""
footer_art = ""
if os.path.exists(conf_path):
    conf = open(conf_path).read()
    for var, key in [("_ba_header", "header"), ("_ba_footer", "footer")]:
        sm = f"##BELLZART_START_{key.upper()}"
        em = f"##BELLZART_END_{key.upper()}"
